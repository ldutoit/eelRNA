...
plot_isoforms<-function(geneofinterest){
  ...
  for (i in 1:dim(sub_counts)[1]){ # go through all the isoforms
    ...
    transcript<-data.frame(Counts= as.numeric(sub_counts[i,]),Type =type)
...
load("countsand_logFCforplottingGene.RData") #Read counts and results are save in DE_analysis_els.Rmd
